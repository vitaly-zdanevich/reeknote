...
use reeknote::geeknote::{self, EvernoteClient, NotesService};
use reeknote::models::{ListItem, Note};
...
fn run(args: Vec<String>) -> Result<()> {
    ...
    match command.as_deref() {
        ...
        Some("create-linked") | Some("edit-linked") => Err(ReeknoteError::Unsupported(format!(
            "{} is not implemented in the Rust client yet",
...
fn handle_settings(storage: &mut Storage, config: &Config, values: ParsedArgs) -> Result<()> {
    ...
    if let Some(value) = values.get("editor") {
        match value {
            ArgValue::String(value) if value == "#GET#" => {
                println!("Current editor is: {}", geeknote::get_editor(Some(storage)));
                return Ok(());
            }
            ArgValue::String(value) => {
                geeknote::set_editor(storage, value)?;
                changed = true;
            }
            _ => {}
        }
        ...
        match value {
            ArgValue::String(value) if value == "#GET#" => {
                println!(
                    ...
                    geeknote::get_extras(Some(storage))
                );
                return Ok(());
            }
            ArgValue::String(value) => {
                geeknote::set_extras(storage, value)?;
                changed = true;
            }
            _ => {}
        }
        ...
        match value {
            ArgValue::String(value) if value == "#GET#" => {
                println!(
                    ...
                    geeknote::get_note_ext(storage)
                );
                return Ok(());
            }
            ArgValue::String(value) => {
                geeknote::set_note_ext(storage, value)?;
                changed = true;
            }
            _ => {}
        }
    ...
    if changed {
        ...
        println!("{}", geeknote::settings_output(storage, config));
    }
    Ok(())
}
...
fn handle_show(storage: &mut Storage, config: &Config, values: ParsedArgs) -> Result<()> {
    ...
    let mut client = make_client(storage, config)?;
    let mut note = resolve_note(storage, &mut client, &note_ref)?;
    if note.content.is_empty() {
        note.content = client.get_note_content(&note.guid)?;
    }

    if arg_bool(&values, "raw") {
        print!("{}", note.content);
    } else {
        let user = storage
            .get_user_info()
...
fn handle_remove(storage: &mut Storage, config: &Config, values: ParsedArgs) -> Result<()> {
    let note_ref = required_string(&values, "note")?;
    let mut client = make_client(storage, config)?;
    let note = resolve_note(storage, &mut client, &note_ref)?;
...
fn handle_dedup(storage: &mut Storage, config: &Config, values: ParsedArgs) -> Result<()> {
    ...
    let candidate_groups = geeknote::duplicate_metadata_groups(&result.notes);
    let mut candidates = Vec::new();
    for metadata in candidate_groups.into_iter().flatten() {
        candidates.push(client.get_note(&metadata.guid)?);
    }
    let duplicate_groups = geeknote::duplicate_content_groups(candidates);
    print!(
        "{}",
...
fn edit_note_text(
    ...
    let editor_command = geeknote::get_editor(Some(storage));
    let note_ext = geeknote::get_note_ext(storage);
...
fn resolve_note(storage: &Storage, client: &mut EdamClient, note_ref: &str) -> Result<Note> {
    ...
    match result.notes.as_slice() {
        ...
        [note] => client.get_note(&note.guid),
        _ => Err(ReeknoteError::InvalidInput(
            "multiple notes match; run `geeknote find` and use the numeric result".to_string(),
        )),
    }
}
